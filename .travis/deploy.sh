...
then
    ...
    ./mvnw --settings .travis/settings.xml versions:set -Dgpg.skip=false -DnewVersion=${TRAVIS_TAG} 1>/dev/null 2>/dev/null
    echo "DEPLOYING RELEASE!"
    ./mvnw --settings .travis/settings.xml clean deploy -Dgpg.skip=false -Dmaven.javadoc.skip=false -Dmaven.source.skip=false -DskipTests=true -B -U
